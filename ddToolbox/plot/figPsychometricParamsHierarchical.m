function figPsychometricParamsHierarchical(mcmc, data)
  ...
  subplot(3,4,5)
  plotPriorPostHist(...
    mcmc.getSamplesAsMatrix({'groupALPHAmuprior'}),...
    mcmc.getSamplesAsMatrix({'groupALPHAmu'}));
  xlabel('\mu_\alpha')

  subplot(3,4,6)
  plotPriorPostHist(...
    mcmc.getSamplesAsMatrix({'groupALPHAsigmaprior'}),...
  ...
  subplot(3,4,7),
  plotPriorPostHist(...
    mcmc.getSamplesAsMatrix({'groupWprior'}),...
    mcmc.getSamplesAsMatrix({'groupW'}));
  xlabel('\omega (mode)')

  subplot(3,4,8),
  plotPriorPostHist(...
    mcmc.getSamplesAsMatrix({'groupKprior'}),...
